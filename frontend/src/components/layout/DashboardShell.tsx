...
export const DashboardShell: React.FC<DashboardShellProps> = ({
  ...
  const getSidebarItems = (): SidebarItem[] => {
    const userRole = user?.role || "BUSINESS_ANALYST";
    const allowedPaths = ROLE_BASED_SIDEBAR[userRole] ?? ROLE_BASED_SIDEBAR["BUSINESS_ANALYST"];
    return ALL_SIDEBAR_ITEMS.filter((item: SidebarItem) => allowedPaths?.includes(item.path));
  };
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Ctrl/Cmd + N - Create new project (navigate to projects)
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-black text-white">
      ...
      {/* ==========================================
          ...
          ========================================== */}
      <aside
        className={`flex flex-col h-full border-r border-white/[0.07] bg-black transition-all duration-200 z-30 select-none fixed lg:static inset-y-0 left-0 ${
          sidebarOpen ? "w-[280px] translate-x-0" : sidebarExpanded ? "w-[220px] lg:translate-x-0" : "w-[52px] lg:translate-x-0"
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-3.5 border-b border-white/[0.07] min-h-[53px]">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="lg:hidden p-1.5 rounded-md hover:bg-white/[0.05] text-gray-600 hover:text-white transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <nav className="flex-1 p-2 overflow-y-auto flex flex-col gap-0.5">
          {isPlatformAdmin ? (
            <button
              ...
              title={!sidebarExpanded && !sidebarOpen ? "Superadmin Control" : undefined}
            >
              <ShieldCheck className={`w-4 h-4 shrink-0 transition-colors ${currentTab === "superadmin" ? "text-white" : "text-gray-600"}`} />
              {(sidebarExpanded || sidebarOpen) && <span className="truncate">Superadmin Panel</span>}
              {currentTab === "superadmin" && (sidebarExpanded || sidebarOpen) && (
            ...
            sidebarItems.map((item: SidebarItem) => {
              ...
              return (
                <button
                  ...
                  title={!sidebarExpanded && !sidebarOpen ? (isLocked(item) ? `${item.name} 🔒 (Locked)` : item.name) : undefined}
                >
                  <Icon className={`w-4 h-4 shrink-0 transition-colors ${isActive ? "text-white" : "text-gray-600"}`} />
                  {(sidebarExpanded || sidebarOpen) && (
                    <span className="truncate">{item.name}</span>
                  )}
                  {isLocked(item) && (sidebarExpanded || sidebarOpen) && (
                    <Lock className="w-3 h-3 text-purple-400/80 ml-auto shrink-0" />
                  )}
                  {isActive && !isLocked(item) && (sidebarExpanded || sidebarOpen) && (
                    <span className="ml-auto w-1 h-1 rounded-full bg-purple-500 shrink-0" />
                  )}
                </button>
              );
        ...
        <div className="px-2 py-3 border-t border-white/[0.07] flex flex-col gap-1.5">
          <div className="flex items-center gap-2 overflow-hidden px-1">
            ...
            {(sidebarExpanded || sidebarOpen) && !sidebarOpen && (
              <button
                onClick={toggleSidebar}
                className="p-1 rounded-md hover:bg-white/[0.05] text-gray-700 hover:text-gray-400 cursor-pointer transition-colors ml-auto hidden lg:block"
          ...
          {!sidebarExpanded && !sidebarOpen && (
            <button
              onClick={toggleSidebar}
              className="p-1.5 mx-auto rounded-md hover:bg-white/[0.05] text-gray-700 hover:text-gray-400 flex items-center justify-center cursor-pointer transition-colors hidden lg:block"
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="h-12 border-b border-white/[0.07] bg-black/70 backdrop-blur-xl flex items-center justify-between px-4 lg:px-5 shrink-0 z-20">
          {/* Left: Mobile Menu & Breadcrumbs & Project Context */}
          <div className="flex items-center gap-2 text-xs text-gray-600 font-medium select-none">
            ...
            {activeProject && (
              <div className="flex items-center gap-1.5 bg-purple-500/10 border border-purple-500/20 text-purple-400 px-2.5 py-1 rounded-md text-[9px] uppercase font-bold tracking-wider shrink-0">
                ...
                <span className="sm:hidden">{activeProject.name.slice(0, 12)}...</span>
              </div>
            )}
            {activeProject && <span className="text-gray-800 font-normal select-none hidden sm:inline">/</span>}
            ...
            {!isPlatformAdmin && (
              <>
                ...
                <button
                  ...
                  className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-[11px] rounded-md bg-gray-900/60 border border-white/[0.06] text-gray-600 outline-none w-36 lg:w-44 text-left placeholder:text-gray-700 hover:border-white/[0.16] hover:text-gray-400 font-medium transition-colors cursor-pointer"
                >
                  <Search className="w-3 h-3 text-gray-700 shrink-0" />
                  <span>Search…</span>
                  <kbd className="ml-auto text-[9px] text-gray-700 font-mono bg-gray-900 border border-white/[0.05] rounded px-1 shrink-0">⌘K</kbd>
                </button>
                {/* Mobile Search Button */}
                <button
                  onClick={() => setIsSearchOpen(true)}
                  className="sm:hidden p-1.5 rounded-md hover:bg-white/[0.05] text-gray-600 hover:text-white transition-colors"
                ...
                <button
                  ...
                  aria-label="Start workspace guide tour"
                >
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-purple-500 rounded-full border border-black" aria-hidden="true" />
                  </button>
                  {notificationsOpen && (
                    <div
      ...
      {/* Contact Widget — Live Chat fallback */}
      <ContactWidget />

      {/* Contextual Help Slide-out Panel */}
      {isHelpOpen && (
        <div className="fixed inset-y-0 right-0 w-80 bg-gray-950 border-l border-white/[0.08] shadow-2xl z-40 flex flex-col select-none text-left" role="dialog" aria-modal="true" aria-labelledby="help-panel-title">
          {/* Drawer Header */}
          <div className="px-4 py-3.5 border-b border-white/[0.08] flex justify-between items-center bg-white/[0.01]">
            <span id="help-panel-title" className="text-xs font-black text-white uppercase tracking-widest flex items-center gap-1.5">
              <HelpCircle className="w-4 h-4 text-purple-400" /> Guidance Center
            </span>
            <button onClick={() => setIsHelpOpen(false)} className="text-gray-500 hover:text-white cursor-pointer transition-colors p-1" aria-label="Close help panel">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>

          {/* Guide Tabs */}
          <div className="grid grid-cols-4 border-b border-white/[0.06] text-[9px] font-bold text-center uppercase tracking-wider text-gray-500" role="tablist">
          ...
          {/* Help Content */}
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 text-xs font-semibold text-gray-400 select-text leading-relaxed" role="tabpanel" id={`help-panel-${activeHelpTab}`}>
